...
function splitArray<T>(arr: Array<T> | [], cols: number) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const index = i % cols;
    if (!result[index]) {
      result[index] = []; // if index not exists, create it with an empty array
    }
    result[index].push(arr[i]); // push the current element to the array at the current index
  }
  return result;
}
...
function Review({ imgSrc, className, ...props }: ReviewProps) {
  ...
  const animationDelay =
    POSSIBLE_ANIMATIONS_DELAYS[Math.floor(Math.random() * POSSIBLE_ANIMATIONS_DELAYS.length)];
